...
function App() {
  function Home() {
    return (
      <div>
        <h1>Pagina Principal de jugadores</h1>
      </div>
    )
  }
  ...
  function JugadoresOnly() {

    const { slug } = useParams();
    const players = jugadores[slug];
    const { name, img, biografia } = players;
    return (
      <div>
        <h2>{name}</h2>
        <img style={{ width: '700px' }} src={img} alt={name} />
        <p>{biografia}</p>
      </div>
    )
  }
    ...
    <BrowserRouter>
      ...
      <Routes>
        ...
        <Route path='/jugadores' element={<Jugadores />}>
          ...
          <Route path=':slug' element={<JugadoresOnly />} />
        </Route>
      </Routes>
    </BrowserRouter>
  );
